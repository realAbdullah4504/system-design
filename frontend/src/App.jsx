...
function App() {
  ...
  const createSession = async () => {
    try {
      const response = await fetch("http://54.198.41.4:3000/api/sessions", {
        method: "POST",
        headers: {
  ...
  const fetchEvents = () => {
    ...
    fetch("http://54.198.41.4:3000/api/events", {
      headers
    })
      .then((res) => res.json())
      .then((data) => {
  ...
  useEffect(() => {
    ...
    const initializeApp = async () => {
      ...
      if (storedSessionId) {
        ...
        try {
          const response = await fetch("http://54.198.41.4:3000/api/current", {
            headers: {
    ...
    const eventSource = new EventSource("http://54.198.41.4:3000/api/events/stream");
  ...
  const sendEvent = async () => {
    try {
      ...
      const response = await fetch("http://54.198.41.4:3000/api/events/send", {
        method: "POST",
        headers,
        body: JSON.stringify({
          type: "test_event",
